...
const ProductBundling = () => {
  ...
  const [fetchingSubcategories, setFetchingSubcategories] = useState(new Set());
  ...
  // Fetch subcategories when main product category changes
  useEffect(() => {
    if (mainProduct.categoryId && 
        !subcategoriesByCategory?.[mainProduct.categoryId] && 
        !fetchingCategories.has(mainProduct.categoryId) && 
        !categoriesLoading) {
      setFetchingCategories(prev => new Set([...prev, mainProduct.categoryId]));
      dispatch(getSubCategoriesByCategory(mainProduct.categoryId)).finally(() => {
        setFetchingCategories(prev => {
          const newSet = new Set(prev);
          newSet.delete(mainProduct.categoryId);
          return newSet;
        });
      });
    }
  }, [mainProduct.categoryId, subcategoriesByCategory, fetchingCategories, categoriesLoading, dispatch]);

  // Fetch subcategories when bundle item categories change
  useEffect(() => {
    bundleItems.forEach(item => {
      if (item.categoryId && 
          !subcategoriesByCategory?.[item.categoryId] && 
          !fetchingCategories.has(item.categoryId) && 
          !categoriesLoading) {
        setFetchingCategories(prev => new Set([...prev, item.categoryId]));
        dispatch(getSubCategoriesByCategory(item.categoryId)).finally(() => {
          setFetchingCategories(prev => {
            const newSet = new Set(prev);
            newSet.delete(item.categoryId);
            return newSet;
          });
        });
      }
    });
  }, [bundleItems, subcategoriesByCategory, fetchingCategories, categoriesLoading, dispatch]);

  // Fetch items when main product subcategory changes
  useEffect(() => {
    if (mainProduct.subCategoryId && 
        !itemsBySubCategory?.[mainProduct.subCategoryId] && 
        !fetchingSubcategories.has(mainProduct.subCategoryId) && 
        !itemsLoading) {
      setFetchingSubcategories(prev => new Set([...prev, mainProduct.subCategoryId]));
      dispatch(getItemsBySubCategory(mainProduct.subCategoryId)).finally(() => {
        setFetchingSubcategories(prev => {
          const newSet = new Set(prev);
          newSet.delete(mainProduct.subCategoryId);
          return newSet;
        });
      });
    }
  }, [mainProduct.subCategoryId, itemsBySubCategory, fetchingSubcategories, itemsLoading, dispatch]);

  // Fetch items when bundle item subcategories change
  useEffect(() => {
    bundleItems.forEach(item => {
      if (item.subCategoryId && 
          !itemsBySubCategory?.[item.subCategoryId] && 
          !fetchingSubcategories.has(item.subCategoryId) && 
          !itemsLoading) {
        setFetchingSubcategories(prev => new Set([...prev, item.subCategoryId]));
        dispatch(getItemsBySubCategory(item.subCategoryId)).finally(() => {
          setFetchingSubcategories(prev => {
            const newSet = new Set(prev);
            newSet.delete(item.subCategoryId);
            return newSet;
          });
        });
      }
    });
  }, [bundleItems, itemsBySubCategory, fetchingSubcategories, itemsLoading, dispatch]);
  ...
  const getSubcategoryOptions = useCallback((categoryId) => {
    ...
    if (!categorySubcategories || !Array.isArray(categorySubcategories)) {
      return []; // Return empty array, let useEffect handle fetching
    }
  ...
  const getItemOptions = useCallback((categoryId, subCategoryId) => {
    ...
    // Check if we have items for this subcategory
    const subcategoryItems = itemsBySubCategory?.[subCategoryId];
    if (!subcategoryItems || !Array.isArray(subcategoryItems)) {
      return []; // Return empty array, let useEffect handle fetching
    }
    
    return subcategoryItems.map(item => ({
        value: item._id,
